...
from .multimodal_encoder.builderMRT import build_vision_tower

from MRT.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN
from .multimodal_encoder.modeling_clipMRT import RT_Vision

class LlavaMetaModel:

    def __init__(self, config, LM_rank, V_rank):
        super(LlavaMetaModel, self).__init__(config, LM_rank, V_rank)
        self.V_rank = V_rank
        self.LM_rank = LM_rank
        self.mm_rt = RT_Vision(
            embed_dim=4096,
            low_rank_dimension=self.V_rank,
            dtype=torch.bfloat16,
            )
        for _, param in self.mm_rt.named_parameters():
            param.requires_grad = True
        if hasattr(config, "mm_vision_tower"):
            self.vision_tower = build_vision_tower(config, self.V_rank, delay_load=True)
    ...
    def initialize_vision_modules(self, model_args, V_rank, fsdp=None):
        ...
        vision_tower = build_vision_tower(model_args, V_rank)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        ...
        image_features = self.get_model().mm_rt(image_features)
