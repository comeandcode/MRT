...
class RT_LM(nn.Module):
    def __init__(self, embed_dim: int, low_rank_dimension: int, dropout: float = 0.0, 
        ...
        self.learned_source = nn.Linear(embed_dim, low_rank_dimension).to(dtype)
        self.dropout = nn.Dropout(dropout)
        self.act_fn = ACT2FN["linear"] if act_fn is None else ACT2FN[act_fn]
        self.dtype = dtype
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig, LM_rank=0, V_rank=6, **kwargs):
        ...
        if self.LM_rank != 0:
            ...
            for _, param in self.prefix_lm_rt.named_parameters():
                param.requires_grad = True
            for _, param in self.suffix_lm_rt.named_parameters():
                param.requires_grad = True
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if (intervention_locations is not None) and (self.LM_rank != 0):
                if isinstance(intervention_locations[0], list) and len(intervention_locations) == hidden_states.shape[0]:
                    # Create a new tensor to store results
                    new_hidden_states = torch.zeros_like(hidden_states)
                    
                    for batch_idx, sample_locations in enumerate(intervention_locations):
                        if sample_locations is not None and len(sample_locations) >= 2:
                            # Apply representation tuning to this batch sample
                            modified_sample = self.apply_representation_tuning(
                                hidden_states[batch_idx:batch_idx+1], idx, sample_locations
                            )
                            new_hidden_states[batch_idx:batch_idx+1] = modified_sample
                        else:
                            # Keep original for this batch sample
                            new_hidden_states[batch_idx:batch_idx+1] = hidden_states[batch_idx:batch_idx+1]
                    
                    hidden_states = new_hidden_states
                else:
                    # intervention_locations is a single location for the whole batch
                    hidden_states = self.apply_representation_tuning(
                        hidden_states, idx, intervention_locations
                    )
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def __init__(self, config,PT_len, VIT_PT_len):
        ...
        print(f"LlamaForCausalLM:{self.lm_head.weight}")
